...
def get_point(points, alpha, deg=None):
    ...
    if deg == 0:
        return points[0]
    return Vec2d.add(Vec2d.mul(points[deg], alpha), Vec2d.mul(get_point(points, alpha, deg - 1), 1 - alpha))
...
def get_knot(points, count):
    ...
    for i in range(-2, len(points) - 2):
        ptn = []
        ptn.append(Vec2d.mul(Vec2d.add(points[i], points[i + 1]), 0.5))
